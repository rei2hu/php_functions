...
git_status() {
    ...
    if [ ! $hash == "" ]; then
        ...
        local added_changes=$(git diff --cached --name-only --diff-filter=A 2> /dev/null | wc -l)
        ...
        if [ ! "0$total_changes" -eq 0 ]; then
            content+="  [\e[32m$added_changes\e[30m/\e[33m$((total_changes-added_changes))\e[30m]"
        fi
        # if untracked file, put asterisk
        if [ ! $(git ls-files --other --directory --exclude-standard 2>/dev/null | sed q1) == "" ]; then
            content+="*"
        fi
    fi
    echo -e $content
